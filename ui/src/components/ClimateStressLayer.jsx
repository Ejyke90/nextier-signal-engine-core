...
const ClimateStressLayer = ({ enabled }) => {
  const map = useMap()
  const [climateData, setClimateData] = useState({ type: 'FeatureCollection', features: [] })

  useEffect(() => {
    const loadClimateIndicators = async () => {
      try {
        const response = await fetch('/data/climate_indicators.geojson')
        if (!response.ok) {
        ...
        // Fallback to empty data or mock data if needed
        setClimateData({ type: 'FeatureCollection', features: [] })
      }
    }

    loadClimateIndicators()
  }, [])
  ...
  useEffect(() => {
    ...
    if (climateData.features.length > 0) {
      geoJsonLayer = L.geoJSON(climateData, {
        style: (feature) => {
